...
Voici ma solution en baktracking pour cet exercice :
...
![img logisim]({{ site.baseurl }}/images/2018-11-battledev/logisim.png)
...
Pour faire simple, une instance de 2-SAT sera toujours de la forme suivante :
`(¬a ∨ b) ∧ (c ∨ d) ∧ (¬a ∨ d) ∧ (¬c ∨ ¬d) ∧ (¬b ∨ c)`
...
Vérifier la satisfaisabilité d'un problème 2-SAT revient à vérifier qu'il n'y a aucun littéral tel que `x` implique `¬x` et en même temps `¬x` implique  `x`. Si aucun des littéraux ne pose de telle contradiction, alors il existera forcément une solution que l'on peut déterminer simplement.
...
Une fois que l'on a déterminé la valeur que doit prendre `a`, on va parcourir le graphe d'implication, ce qui va déterminer toutes les autres valeurs que l'on va forcer. En reprenant l'exemple de graphe d'implication ci-dessus, forcer `¬x0` revient à aussi forcer `¬x3`, `¬x5` et `x6`.
...
Notre problème s'apparente donc à un 2-SAT car les clauses comportent 2 littéraux, mais la Forme Normale Conjonctive interdit les clauses de forme `¬ (¬e0 ∧ ¬e1)`. Pour se mettre dans les règles de la FNC, il faut absolument des clauses `(littéral1 ∨ littéral2)`. Heureusement, les lois de De Morgan nous permettent de transformer nos clauses pour les rendre valides en regard de la FNC : `¬(x ∧ y)` est strictement équivalent à `(¬x ∨ ¬y)`, ce qui nous donne donc une formule 2-SAT valide. La formule qui correspond au dessin est
`(e0 ∨ e1) ∧ (e0 ∨ e2) ∧ (e0 ∨ e3) ∧ (¬e0 ∨ ¬e1) ∧ (¬e0 ∨ ¬e2) ∧ (e1 ∨ e3) ∧ (e1 ∨ ¬e3) ∧ (¬e1 ∨ ¬e2)`
