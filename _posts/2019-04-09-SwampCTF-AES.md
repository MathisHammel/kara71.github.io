...
I'm already digressing, let's get back to ze moutons. The python code is pretty straightforward without any obfuscation, it uses the common package PyCrypto. We can already sense this challenge will be pure crypto, without much reverse engineering. The right mix of crypto and reverse makes for the best CTF challenges on earth, but is hard to achieve.

We can see the code uses AES, which is a famous cryptographic function. But the most important part here is the use of `AES.MODE_CBC`, which is where the vulnerability lies. I reckon I've probably lost half of the readers by going into AES and CBC, so let's backtrack a little and explain what all of these two words mean.

## AES - Advanced Encryption Standard

AES is probably the only encryption algorithm that is used more than RSA (both are extremely useful but do not serve the same purposes).

AES has two main defining properties :

 - **Block cipher** : This algorithm takes a fixed size input (in this case, 16 bytes) called the plaintext, and spits an output of the same size called the ciphertext. If you're familiar with RSA, that one isn't a block cipher because it accepts numbers of any size smaller than N.
...
It's safe to assume that we are not breaking the AES algorithm itself, but rather its implementation (choice of keys, compression, padding, ...). Actually, the encryption function is only the brick the challenge is built with, but we should be focusing on attacking open doors instead of breaking through the walls. Remember the `AES.MODE_CBC` ? It defines how the data is actually encrypted with the block cipher. Let's have a deeper look into it.
...
Well, that's quite easy isn't it ? We can just break the text into smaller chunks of 16 bytes and encrypt them one by one !

Congratulations, we ust invented something called ECB. ECB is the retarded cousin of CBC, and here's why: let's imagine we are encrypting the message `Today's code: 4975384264852. Bye` using AES-ECB and sharing it with an ally :

![ecb1]({{ site.baseurl }}/images/2019-04-swamp/ecb1.png)

The encryption is made correctly, and there is actually no way for an enemy intercepting the encrypted message to recover the plaintext. However, fast forward a few weeks, we're sending a different code `Today's code: 4935412269921. Bye` encrypted with the same key.

![ecb2]({{ site.baseurl }}/images/2019-04-swamp/ecb2.png)

If you have a closer look at the ciphertext on the left, you will notice it's exactly the same as the previous one, which might reveal some precious information on your code if someone intercepts the message and knows what code you have sent previously. The supposedly perfect cryptosystem we invented has turned into a mediocre cryptosystem leaking information. Using ECB is the easiest and fastest way to chain blocks with block ciphers, but it's recommended to use a better way of encrypting multiple blocks, such as CBC.
...
It's actually quite simple : a random value (called the initialization vector or IV) is mixed with the first plaintext before encryption. Then, we mix the first ciphertext with the second plaintext and encrypt the mix, and so on. In practice, the mixing is done with XOR, which is a reversible transformation. If the IV is chosen randomly and never reused, the cascading property (aka butterfly effect) of AES makes the cipher unbreakable.

![cbc1]({{ site.baseurl }}/images/2019-04-swamp/cbc1.png)

The IV is transmitted along with the ciphertext, making each transmission 16 bytes longer. The decryption algorithm is more complicated than the straightforward ECB, but we can reverse the operations made in the encryption. The mixing operation has to be reversible, because we need to untangle the ciphertext and the plaintext from the AES decryption's output.

The inverse of XOR is actually XOR itself and has several other properties which mean that if we XOR the decrypted data = `plaintext_block[i] xor ciphertext_block[i-1]` with `ciphertext_block[i-1]`, we get
...
Now, we have all wee need to successfully execute an attack on the cryptosystem. I won't get into PKCS7 padding which is also implemented in the challenge, the only thing you have to know is that it's used to make the ciphertext length a multiple of 16 so the data can nicely be split into blocks without the last one being too short.

## Applied attack on bad AES-CBC

Now if you remember what I said, the IV has to be chosen perfectly randomly and never reused. But have a look at the implementation :
...
Not only is the IV reused, it also contains a value that should be kept secret ! To attack this, we don't even need to use the encrypt function - let's look at what happens if we decrypt a fake ciphertext full of null bytes :
...
Outputs 1, 2 and 3 are different because they are not XORed with the same values, but the data right at the exit of each block is exactly the same.

Output 1 corresponds to `decryptAES(0000000000000000, key) xor IV`, whereas outputs 2 and 3 are `decryptAES(0000000000000000, key) xor 0000000000000000`. Let's verify this in practice (I added spaces between blocks for clarity).
